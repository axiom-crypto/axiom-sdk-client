...
export async function validateTxSubquery(
  ...
  const txHash = await getTxHash(provider, subquery.blockNumber, subquery.txIdx);
  if (!txHash) {
    console.error(`Unable to get txHash from blockNumber: ${subquery.blockNumber} and txIdx: ${subquery.txIdx}`);
    return false;
  }
  const tx = await getRawTransaction(provider, txHash);
  if (!tx) {
    console.error(`Unable to get transaction from txHash: ${txHash}`);
    return false;
  }
...
export async function validateReceiptSubquery(
  ...
  const rc = await getRawReceipt(provider, txHash);
  if (!rc) {
    console.error(`Unable to get receipt from txHash: ${txHash}`);
    return false;
  }
