import { buildCircuit } from "../../src/template/buildCircuit";
  ...
  for (let [folder, files] of Object.entries(fileMap)) {
    for (let file of files) {
      ...
      test(`Test ${folder}: ${inputFile}`, async () => {
        ...
        const circuit = buildCircuit(inputFile);

        // Write the typescript circuit
        const circuitPath = `${outputFileBase}.circuit.ts`;
        ...
        const compileCmd = `npx axiom compile "${circuitPath}" --function circuit --output "${buildFile}"`;
        ...
        const runCmd = `npx axiom run "${circuitPath}" --function circuit --build "${buildFile}" --output "${outputFile}"`;
