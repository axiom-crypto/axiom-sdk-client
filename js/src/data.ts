...
import { PrepData, fetchDataQuery, getCircuitValue256FromCircuitValue, getCircuitValue256Witness, getCircuitValueConstant, getCircuitValueWitness } from "./utils";
...
export class AxiomData {
  ...
  constructor(halo2Wasm: Halo2Wasm, halo2Lib: Halo2LibWasm, provider: JsonRpcProvider, results?: { [key: string]: string }) {
  ...
  private prepData<T extends object>(type: DataSubqueryType): PrepData<T> {
    ...
    return async (subquery, queryArr) => {
      let dataSubquery: DataSubquery = { subqueryData: subquery, type };
      let val = await fetchDataQuery(this._provider, dataSubquery, this._results);
      this._dataQuery.push(dataSubquery);
      ...
      for (let i = queryArr.length; i < 13; i++) {
        paddedQueryArr.push(getCircuitValueWitness(this._halo2Lib, 0));
      }
      paddedQueryArr.push(witness.hi());
      paddedQueryArr.push(witness.lo());

      for (let queryCell of paddedQueryArr) {
        this._halo2Lib.make_public(this._halo2Wasm, queryCell.cell(), 1);
      }
